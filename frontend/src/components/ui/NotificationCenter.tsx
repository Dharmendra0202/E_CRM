...
const TYPE_ICON: Record<string, { icon: React.ReactNode; color: string }> = {
  ...
  GENERAL: { icon: <CalendarDays size={14} />, color: "hsl(271,91%,60%)" },
...
export function NotificationCenter({ invoiceCount, studentCount, homeworkCount }: NotificationCenterProps) {
  ...
  const derivedItems = [
    ...
    ...(studentCount > 0 ? [{ id: "stu", icon: <Users2 size={14} />, color: "hsl(271,91%,60%)", title: `${studentCount} new student${studentCount > 1 ? "s" : ""} this month`, desc: "Review enrollments", time: "Today", isRead: true }] : []),
        ...
        >
          ...
          <div style={{ maxHeight: "300px", overflowY: "auto" }}>
            {notifications.length === 0 ? (
              ...
              <div key={notif.id} style={{ display: "flex", gap: "12px", padding: "12px 18px", borderBottom: "1px solid var(--border-glass)", transition: "background 0.15s", cursor: "pointer", background: notif.isRead ? "transparent" : "hsla(328,100%,54%,0.03)" }}
                onMouseEnter={(e) => (e.currentTarget.style.background = "hsla(328,100%,54%,0.06)")}
                onMouseLeave={(e) => (e.currentTarget.style.background = notif.isRead ? "transparent" : "hsla(328,100%,54%,0.03)")}>
